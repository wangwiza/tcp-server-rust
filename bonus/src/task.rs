...
pub const TOTAL_TASK_TYPE: usize = 2;
...
impl Task {
    ...
    fn do_cpu_intensive_task(seed: u64) -> u8 {
        ...
        let total_rounds = Self::MULTIPLIERS[rng.next_u64() as usize % 16] * 4 * 1024;

        let mut dep = 0u8;

        for _ in 0..total_rounds {
            let index = (rng.next_u64() as usize + dep as usize) % Self::DATA_SIZE;
            data[index] = data[index].wrapping_add(rng.next_u64() as u8);
    ...
    fn do_io_intensive_task(seed: u64) -> u8 {
        ...
        let duration: usize = rng.random_range(0..=MAX_IO_DURATION_MS);
